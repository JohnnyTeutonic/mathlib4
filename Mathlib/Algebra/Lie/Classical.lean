...
theorem finrank_sl [Fintype n] [Nonempty n] (K : Type*) [Field K] :
    Module.finrank K (sl n K) = Fintype.card n ^ 2 - 1 := by
  change Module.finrank K (sl n K).toSubmodule = _
  simp only [sl]
  rw [sq]
  have := (Matrix.traceLinearMap n K K).finrank_range_add_finrank_ker
  rw [show (Matrix.traceLinearMap n K K).range = ⊤ from
      LinearMap.range_eq_top.mpr fun r => Matrix.trace_surjective r,
    finrank_top, finrank_self, finrank_matrix, finrank_self, mul_one] at this
  omega
